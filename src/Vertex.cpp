...
/*
 * A chunk of `bytes` holds at most bytes/EDGE_BYTES edges and each edge
 * occupies two adjacency entries, so 2*bytes/EDGE_BYTES entries is the whole
 * chunk's adjacency.  Those entries are spread over several size classes, so
 * allowing every class a slab that large would overshoot; a quarter of it per
 * class keeps the total in the region of one chunk while still amortising the
 * busy classes.
 */
...
AdjEntry * AdjArena::acquire( largeint want , largeint & cap )
{
  ...
  /*
   * Beyond the largest class there is no slab to carve from.  Silently handing
   * back a block smaller than asked for would corrupt the graph, so take it
   * from the allocator directly and record it so clear() frees it.
   */
  if( cap < want ){
    cap = want;
...
bool Vertex::is_dep_vertex()
{
  return dep_vertex;
}

largeint Vertex::get_idx()
{
  return idx;
}

largeint Vertex::get_owner_idx()
{
  return owner_idx;
}
...
Edge * Vertex::to( Vertex * tgt )
{
  Adjacency::iterator it = out_edges.find( tgt->idx );
...
largeint Vertex::eliminate( EdgeArena & arena , AdjArena & adj )
{
  ...
  largeint P_j  =  in_edges.size();
  largeint S_j  =  out_edges.size();
  largeint cost =  P_j*S_j;
